...
function renderPendingSection(container, { onComplete, onRefresh }) {
  ...
  titleTile.innerHTML = `
    <span style="font-family:var(--font-display); font-size:20px; text-transform:uppercase; letter-spacing:.04em; color:rgba(197,160,40,0.6);">
      Pending Trades${trades.length > 0 ? ` <span style="color:rgba(197,160,40,0.6);">(${trades.length})</span>` : ''}
    </span>
  `;
...
export async function mountSwapAnalyser(container) {
  ...
  genTitleTile.innerHTML = `<span style="font-family:var(--font-display); font-size:20px; text-transform:uppercase; letter-spacing:.04em; color:rgba(197,160,40,0.6);">Generate Trade</span>`;
